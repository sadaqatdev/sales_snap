...
class Extractor {
  ...
  getAmazonTitle(String dom, Function callback) {
    List<String> titles = [];
    HtmlDocument document = parseHtmlDocument(dom);

    Selectors.amazonTitles.forEach((String query) {
      print(query);
  ...
  getamazonPrice(String dom, Function callback) {
    List<String> prices = [];

    HtmlDocument document = parseHtmlDocument(dom);
    Element priceElement;

    Selectors.amazonPrices.forEach((String query) {
      Element element = document.body.querySelector('#$query');
      print(element?.innerHtml);
      if (element != null) {
        print(element.attributes.toString());
        print(element.innerHtml);
        priceElement = element;
        prices.add(element.innerHtml);
      }
    });
    callback(prices, priceElement);
  }

  getAmazonImages(String dom, Function callback) {
    List<String> images = [];
    HtmlDocument document = parseHtmlDocument(dom);
    Selectors.amazonImages.forEach((String query) {
      Element element = document.body.querySelector(query);
      while (element == null) {
